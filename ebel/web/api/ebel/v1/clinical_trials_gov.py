...
def get_ct_by_mesh_term():
    ...
    query = (
        RDBMS.get_session()
        .query(ct.MeshTerm)
        .filter(ct.MeshTerm.mesh_term.like(mesh_term))
    )
...
def get_ct_by_keyword():
    ...
    query = (
        RDBMS.get_session().query(ct.Keyword).filter(ct.Keyword.keyword.like(keyword))
    )
...
def get_ct_by_condition():
    ...
    query = (
        RDBMS.get_session()
        .query(ct.Condition)
        .filter(ct.Condition.condition.like(condition))
    )
...
def get_mesh_term_starts_with():
    ...
    return _get_terms_from_model_starts_with(
        form_field="mesh_term", sa_column=ct.MeshTerm.mesh_term
    )
...
def get_keyword_starts_with():
    ...
    return _get_terms_from_model_starts_with(
        form_field="keyword", sa_column=ct.Keyword.keyword
    )
...
def get_condition_starts_with():
    ...
    return _get_terms_from_model_starts_with(
        form_field="condition", sa_column=ct.Condition.condition
    )
