...
def get_medgen_by_pmid():
    ...
    q = session.query(n.cui, n.name).join(p).filter_by(pmid=request.args.get('pmid'))
    return _get_paginated_query_result(q, return_dict=True)


def get_go_by_pmid():
    """Get gene ontology by PMID."""
    q = session.query(ncbi.NcbiGeneGo).join(ncbi.NcbiGeneGoPmid).filter_by(pmid=request.args.get('pmid'))
